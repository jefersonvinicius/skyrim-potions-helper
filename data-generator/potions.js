...
const POTIONS_PAGE = "https://elderscrolls.fandom.com/wiki/Created_Potions_(Skyrim)";
const OUTPUT_FILE = path.join(currentPath, "..", "potions.json")
...
async function main() {
    ...
    const result = await page.evaluate(() => {
        ...
        const rowsNodes = potionsTable.querySelector('tbody').querySelectorAll('tr')
        const potionsResult = {}
        rowsNodes.forEach(row => {
            const [potionColumn, ingredientsColumn] = row.querySelectorAll('td')
            potionsResult[potionColumn.textContent] = ingredientsColumn.textContent.split('\n');
        })
    ...
    Object.entries(result).forEach(([potion, ingredients]) => {
        cleanResult[cleanDOMText(potion)] = ingredients.map(cleanDOMText).filter(ingredient => !!ingredient)
    })
